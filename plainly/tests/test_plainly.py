...
from plainly import Interpreter, interpret, run_file
...
"""
    ...
    def test_run_file_executes_a_plainly_source_file(self):
        with tempfile.NamedTemporaryFile("w", suffix=".plainly", delete=False) as handle:
            handle.write('say "hello from file".')
            path = handle.name

        try:
            self.assertEqual(run_file(path, gui_enabled=False), ["hello from file"])
        finally:
            os.unlink(path)

    def test_python_can_execute_plainly_files_directly(self):
        with tempfile.NamedTemporaryFile("w", suffix=".plainly", dir=os.getcwd(), delete=False) as handle:
            handle.write('say "hello from direct exec".')
            path = handle.name

        try:
            completed = subprocess.run([sys.executable, path], cwd=os.getcwd(), capture_output=True, text=True, check=False)
            self.assertEqual(completed.returncode, 0, completed.stderr)
            self.assertIn("hello from direct exec", completed.stdout)
        finally:
            os.unlink(path)
