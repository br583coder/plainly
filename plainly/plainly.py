...
def run_file(path: str, input_fn: Optional[Callable[[str], str]] = None, gui_enabled: bool = True) -> List[str]:
    with open(path, "r", encoding="utf-8-sig") as handle:
        interpreter = Interpreter(input_fn=input_fn, gui_enabled=gui_enabled)
        return interpreter.interpret(handle.read())
...
if __name__ == "__main__":
    ...
    run_file(sys.argv[1], gui_enabled=True)
